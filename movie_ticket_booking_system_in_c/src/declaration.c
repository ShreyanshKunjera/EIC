...
int choice()        // Ask the user one of the 5 actions
{
        int choice;
...
int print_available_seats(int array[],int selection,int time_of_movie)  // to print available seats(Action 1)
{
        ...
        if(selection==1 && time_of_movie==1)
        {
        	seat_no=1;  // if it is 1st screen and 1st time then start with 1st index in array of seats
        }
        else if(selection==1 && time_of_movie==2)
        {
        	seat_no=51;
        }
        else if(selection==1 && time_of_movie==3)
        {
        	seat_no=101;
        }
        else if(selection==2 && time_of_movie==1)
        {
        	seat_no=151;
        }
        else if(selection==2 && time_of_movie==2)
        {
        	seat_no=201;
        }
        else if(selection==2 && time_of_movie==3)
        {
        	seat_no=251;
        }
...
void reservation(int array[],int price,int selection, int time1) // Function for booking the ticket
{
        ...
        int orig_seat_no=print_available_seats(array,selection,time1); // to get value of orig_seat_no from available_seat function
	...
	scanf("%s",person[count].name);
        ...
        for(int i=0;i<num_of_seats;i++)
        {
        	scanf("%s",input_of_seat);
            	...
            	{
                	printf("Give valid seat no as shown in available seat (From A1 to A5, B1 to B5, ... till J1 to J5)\n");
                	scanf("%s",input_of_seat);
            	}
        ...
        amount=num_of_seats*(selection==1?400:600);
        printf("\n");
